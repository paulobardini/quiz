-- ============================================
-- SQL para inserir conteúdo premium dos perfis de CLAREZA
-- ============================================

-- CLAREZA MUITO BAIXA (clareza_muito_baixa)
DO $$
DECLARE
  v_profile_id uuid;
  v_blocks jsonb := '[
...
BEGIN
  ...
  FROM quiz_profiles
  WHERE key = 'clareza_muito_baixa';
  
  -- Se não encontrou o perfil, sair
  IF v_profile_id IS NULL THEN
    RAISE EXCEPTION 'Perfil clareza_muito_baixa não encontrado';
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM premium_report_content WHERE profile_id = v_profile_id) THEN
    ...
    SET 
      title = 'Leitura completa do seu padrão de decisão',
      blocks = v_blocks,
      version = 'v1',
      updated_at = NOW()
  ...
  ELSE
    ...
    VALUES (v_profile_id, 'Leitura completa do seu padrão de decisão', v_blocks, 'v1');
  END IF;
END $$;

-- CLAREZA BAIXA (clareza_baixa)
INSERT INTO premium_report_content (profile_id, title, blocks, version)
SELECT 
  qp.id as profile_id,
...
WHERE qp.key = 'clareza_baixa'
ON CONFLICT (profile_id, version) DO UPDATE SET
  title = EXCLUDED.title,
  blocks = EXCLUDED.blocks,
  updated_at = NOW();

-- CLAREZA MÉDIA (clareza_media)
INSERT INTO premium_report_content (profile_id, title, blocks, version)
SELECT 
  qp.id as profile_id,
  'Leitura completa do seu padrão de decisão' as title,
  ...
  'v1' as version
FROM quiz_profiles qp
WHERE qp.key = 'clareza_media'
ON CONFLICT (profile_id, version) DO UPDATE SET
  title = EXCLUDED.title,
  blocks = EXCLUDED.blocks,
  updated_at = NOW();

-- CLAREZA ALTA (clareza_alta)
INSERT INTO premium_report_content (profile_id, title, blocks, version)
SELECT 
  qp.id as profile_id,
  'Leitura completa do seu padrão de decisão' as title,
  ...
  'v1' as version
FROM quiz_profiles qp
WHERE qp.key = 'clareza_alta'
ON CONFLICT (profile_id, version) DO UPDATE SET
  title = EXCLUDED.title,
  blocks = EXCLUDED.blocks,
  updated_at = NOW();

-- CLAREZA MUITO ALTA (clareza_muito_alta)
INSERT INTO premium_report_content (profile_id, title, blocks, version)
SELECT 
  qp.id as profile_id,
  'Leitura completa do seu padrão de decisão' as title,
  ...
  'v1' as version
FROM quiz_profiles qp
WHERE qp.key = 'clareza_muito_alta'
ON CONFLICT (profile_id, version) DO UPDATE SET
  title = EXCLUDED.title,
  blocks = EXCLUDED.blocks,
  updated_at = NOW();
